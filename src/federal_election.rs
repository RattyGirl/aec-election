...
async fn read_aec_light(election_id: &String, db: &mut MySQLDB, mut zipfile: &mut ZipArchive<Cursor<Vec<u8>>>) {
    let mut results_string: String = String::new();
    zipfile
        .by_name(format!("xml/aec-mediafeed-results-detailed-light-{}.xml", election_id).as_str())
        .unwrap()
    ...
    for election in rootnode.get_child_ignore_ns("Results").unwrap().get_children_ignore_ns("Election") {
        if let Some(house) = election.get_child_ignore_ns("House") {
            //house election
            let contests = house.get_child_ignore_ns("Contests").unwrap().get_children_ignore_ns("Contest");
            for contest in contests {
                let contest_id = contest.get_child_ignore_ns("ContestIdentifier").unwrap().attr("Id").unwrap().to_string();
                for polling_place in contest.get_child_ignore_ns("PollingPlaces").unwrap().get_children_ignore_ns("PollingPlace") {
                    let candidate_vec: Vec<String> = polling_place.get_child_ignore_ns("FirstPreferences").unwrap().get_children_ignore_ns("Candidate").iter().map(|candidate| {
                        let vote_struct = PrimaryVote {
                            event_id: election_id.clone(),
                            contest_id: contest_id.clone(),
                            polling_place_id: polling_place.get_child_ignore_ns("PollingPlaceIdentifier").unwrap().attr("Id").unwrap().to_string(),
                            candidate_id: candidate.get_child_ignore_ns("CandidateIdentifier").unwrap().attr("Id").unwrap().to_string(),
                            votes: candidate.get_child_ignore_ns("Votes").unwrap().text().parse().unwrap(),
                        };
                        format!("($${}$$, $${}$$, $${}$$, $${}$$, $${}$$)", vote_struct.event_id, vote_struct.contest_id, vote_struct.polling_place_id, vote_struct.candidate_id, vote_struct.votes)
                    }).collect();
...
async fn read_results_preload(election_id: &String, db: &mut MySQLDB, mut zipfile: &mut ZipArchive<Cursor<Vec<u8>>>) {
    let mut results_string: String = String::new();
    zipfile
        .by_name(format!("xml/aec-mediafeed-results-detailed-preload-{}.xml", election_id).as_str())
        .unwrap()
        .read_to_string(&mut results_string)
        .unwrap();
    let mut results_string: Vec<&str> = results_string.split('\n').collect();
...
async fn read_aec_pollingdistricts(election_id: &String, db: &mut MySQLDB, mut zipfile: &mut ZipArchive<Cursor<Vec<u8>>>) {
    let mut results_string: String = String::new();
    zipfile
    ...
    let rootnode: Element = Element::from_str(results_string.as_str()).unwrap();
    let pollingplace_structs: Vec<String> = rootnode.get_child_ignore_ns("PollingDistrictList").unwrap().get_children_ignore_ns("PollingDistrict").iter().map(|districtelement| {
        let district_id = districtelement.get_child_ignore_ns("PollingDistrictIdentifier").unwrap().attr("Id").unwrap().to_string();
        let pollingplace_structs: Vec<String> = districtelement.get_child_ignore_ns("PollingPlaces").unwrap().get_children_ignore_ns("PollingPlace").iter().map(|pollingplace_element| {
            let pollingplacestruct = PollingPlace {
                event_id: election_id.to_string(),
                district_id: district_id.clone(),
                id: pollingplace_element.get_child_ignore_ns("PollingPlaceIdentifier").unwrap().attr("Id").unwrap().to_string(),
                name: pollingplace_element.get_child_ignore_ns("PollingPlaceIdentifier").unwrap().attr("Name").unwrap().to_string(),
            };
            ...
            format!("($${}$$, $${}$$, $${}$$, $${}$$)", pollingplacestruct.event_id, pollingplacestruct.district_id, pollingplacestruct.id, pollingplacestruct.name)
        }).collect();
...
async fn read_eml_110_event_xml(election_id: &String, db: &mut MySQLDB, mut zipfile: &mut ZipArchive<Cursor<Vec<u8>>>) {
    let mut results_string: String = String::new();
    zipfile
        ...
        .get_children_ignore_ns("Election")
    {
        ...
        for contestelement  in electionelement.get_children_ignore_ns("Contest") {
            let contest_struct = Contest {
                ...
                id: contestelement.get_child_ignore_ns("ContestIdentifier").unwrap().attr("Id").unwrap().to_string(),
                shortcode: contestelement.get_child_ignore_ns("ContestIdentifier").unwrap().attr("ShortCode").unwrap_or("").to_string(),
                name: contestelement.get_child_ignore_ns("ContestIdentifier").unwrap().get_child_ignore_ns("ContestName").unwrap().text(),
            };
...
async fn read_eml_230_candidates(election_id: &String, db: &mut MySQLDB, mut zipfile: &mut ZipArchive<Cursor<Vec<u8>>>) {
    let mut results_string: String = String::new();
        ...
        .get_children_ignore_ns("Election")
    {
        ...
        for contest in electionelement.get_children_ignore_ns("Contest") {
            let contest_id = contest.get_child_ignore_ns("ContestIdentifier").unwrap().attr("Id").unwrap();
            for candidate in contest.get_children_ignore_ns("Candidate") {
                let candidate = Candidate {
                    ...
                    id: candidate.get_child_ignore_ns("CandidateIdentifier").unwrap().attr("Id").unwrap().to_string(),
                    name: candidate.get_child_ignore_ns("CandidateIdentifier").unwrap().get_child_ignore_ns("CandidateName").unwrap().text(),
                    affiliation: match candidate.get_child_ignore_ns("Affiliation") {
                        None => "".to_string(),
                        Some(affiliation) => affiliation.get_child_ignore_ns("AffiliationIdentifier").unwrap().attr("ShortCode").unwrap().to_string()
                    },
                };
